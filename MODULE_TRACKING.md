...
1. ~~Pay Now fix~~ — FIXED in Round 5. Added deduplication check, cancellation check, session selection with attendance summary, date picker, and improved UI with ShellDialog.
...
7. **Family accounts** — deferred. Requires new tables (families, family_members), manual linking UI, family-level discount application.

8. **Attendance-based billing cycles** — deferred. Requires billing_cycles table, per-student cycle tracking, cycle completion logic. Note: School closures vs single-session cancellations need clearer distinction in how they affect a student's billing cycle count and whether a make-up session is auto-added.

9. **Refunds and credit notes** — deferred. Requires refund workflow (cash refund vs credit note), automatic credit balance application, credit_note tracking.

10. **KPI dashboard** — deferred. Requires free-date-range-driven KPI cards (total revenue, outstanding, collection rate, net balance, avg days-to-pay), bar chart library.

11. **Bulk payment recording** — deferred. Requires multi-student selection, per-student breakdown, atomic batch transaction.

12. **Student/Teacher PDF statements** — deferred. Requires full-history PDF generation per student/teacher.

13. **PDF receipt with QR code** — deferred. Requires QR generation library, monthly printable receipt per student.

14. **Unbilled-but-active indicator** — deferred. Requires tracking which enrolled students have no session_charge for current cycle.
